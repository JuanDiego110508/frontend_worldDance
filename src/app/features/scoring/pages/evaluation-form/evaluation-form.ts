...
export class EvaluationFormComponent implements OnInit {
  ...
  criteria = signal<CriterionFormRow[]>([
    { name: 'Técnica', percentage: 40, score: 0 },
    { name: 'Expresión Artística', percentage: 30, score: 0 },
    { name: 'Ejecución y Sincronía', percentage: 30, score: 0 }
  ]);
  ...
  onSubmit(): void {
    ...
    obs.subscribe({
      ...
      error: (err) => {
        // err.message is generated by handleError in the service, but let's check if it's a 409
        if (err.message && err.message.includes('409')) {
           this.errorMessage.set('Ya has calificado a este participante previamente. No puedes volver a evaluarlo.');
        } else {
           this.errorMessage.set(err.message || 'Error al guardar la evaluación. Es posible que ya exista.');
        }
        this.isSubmitting.set(false);
      }
    });
